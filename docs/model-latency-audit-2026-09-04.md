...
This is a source audit and local serialization benchmark, not a live comparison of provider
response times. It covers Send preparation, provider startup/history, context, tools, tracing,
...
| --- | --- | --- | --- |
| High | Antigravity process startup | `antigravity-session.ts` sends an internal READY prompt on every new process. The user's prompt waits for that model turn to finish. Parking, restart, or model changes can repeat the cost. | Verify eager MCP/browser tools load without this model turn, then remove it or replace it with a protocol readiness signal. The source documents it as an MCP initialization workaround; this audit did not establish that removing it preserves tool availability. Corrected the guide's inaccurate claim that it was off the Send path. |
...
| Medium | Cursor cold catalog startup | When a fresh catalog must come from the agent, its initial session load and the subsequent transcript replay still perform separate loads. | Collect replay notifications during the first load. `docs/cursor.md` records an earlier approximately 0.45-second load measurement; it was not remeasured here. This is distinct from the fixed discarded-setup bug. |
...
| Aggregate telemetry | `tools/telemetry.ts` queues an atomic JSON snapshot write per tool completion. Bursts can queue redundant latest-state snapshots. Writes are asynchronous and not awaited by the model tool, so this is a possible disk/CPU contention source rather than a mandatory per-tool wait. Coalescing pending writes is a small remaining improvement. |
