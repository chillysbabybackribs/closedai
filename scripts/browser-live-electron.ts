...
const targetUrl = process.env.CLOSEDAI_BROWSER_LIVE_URL ?? 'https://www.google.com/'
...
async function verify(): Promise<void> {
  ...
  try {
    const navigate = await registry.call({
      ...
      tool: 'page',
      arguments: { action: 'navigate', url: targetUrl, wait_until: 'load', timeout_ms: 15_000 }
    }, context)
    ...
    const queryJson = jsonOf(query)
    assert.ok(Number(queryJson.matched) >= 1, `expected a search control on ${targetUrl}: ${textOf(query)}`)
    ...
    assert.match(String(evalJson.value?.url), /google/i)
    ...
    assert.ok(
      networkJson.requests?.some((record) => String(record.url).includes('google')),
      'expected a google request in the session log'
    )

    const session = await registry.call({
      namespace: 'embedded_browser',
      tool: 'session',
      arguments: { action: 'fetch', url: targetUrl, max_chars: 4_000 }
    }, context)
    ...
    console.log(JSON.stringify({
      ...
      searchControls: queryJson.matched,
