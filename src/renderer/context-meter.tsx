...
export function ContextMeter({ usage }: { usage: ChatContextUsage }): JSX.Element {
  ...
  return (
    <span className="context-meter" data-level={level} title={detail} aria-label={detail}>
      ...
      <span className="context-meter-detail">{formatTokens(usage.usedTokens)}/{formatTokens(usage.contextWindow)}</span>
    </span>
  )
}
