...
export function registerBrowserDownloadsIpc(
  ...
  ipcMain.handle('browserDownloads:list', () => getDownloads()?.list() ?? [])
  ipcMain.handle('browserDownloads:pause', (_event, id: string) => getDownloads()?.pause(id))
  ipcMain.handle('browserDownloads:resume', (_event, id: string) => getDownloads()?.resume(id))
  ipcMain.handle('browserDownloads:cancel', (_event, id: string) => getDownloads()?.cancel(id))
  ipcMain.handle('browserDownloads:clear', () => getDownloads()?.clear())
  ipcMain.handle('browserDownloads:reveal', (_event, id: string) => {
