...
const INSTRUCTIONS = [
  ...
  'For live-app work, read closedai_app.state for facts and use closedai_app.command for deterministic actions (new chat, send to another pane with await_turn, stop, open, model, browser tabs). Use closedai_app.ui only when the real control must be exercised: list controls scoped by surface, then act by control id and item. Never read renderer source to find controls or selectors; the ids are listed at runtime.',
