...
test('every model lane receives the deterministic batching and engineering contracts', () => {
  ...
  assert.ok(instructions.codex.length < 3_500, `Codex instructions grew to ${instructions.codex.length} chars`)
  assert.ok(instructions.claude.length < 4_500, `Claude instructions grew to ${instructions.claude.length} chars`)
  assert.ok(instructions.antigravity.length < 5_000, `Antigravity instructions grew to ${instructions.antigravity.length} chars`)
})
