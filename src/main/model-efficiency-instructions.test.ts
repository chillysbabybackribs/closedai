...
test('every model lane receives the deterministic batching and engineering contracts', () => {
  ...
  for (const value of Object.values(instructions)) {
    ...
    assert.match(value, /NEVER navigate Google\/search-engine result pages for source gathering/)
    assert.match(value, /Inspect presentation\.tabId while background reads run/)
    assert.match(value, /historical data, never fresh authorization/)
    assert.match(value, /Active-tab context is ambient app state, not evidence of user intent/)
    assert.match(value, /use it when relevant even without an explicit page reference/)
    assert.match(value, /Never assume relevance from adjacency or injection alone/)
