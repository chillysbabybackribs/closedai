...
const NAVIGATION_PROSE = [
  ...
  'The map below is generated and current: read a path off it rather than searching for one. Use closedai_workspace.inspect find for unknown locations; exact symbols include hashed source and styles. For a known path use read with a symbol or range to include related styles and test paths; outline is shape only. Reuse supplied source, read missing ranges, and pass known_hash only for source still in context; stale hashes return fresh content in that call.'
].join('\n')
