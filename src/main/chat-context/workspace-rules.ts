 ...
 * workspace root's policy here. Native file tools handle discovery of any nested policies.
 */
export function workspaceRulesSection(cwd: string): string | null {
  ...
  const clipped = source.length > MAX_WORKSPACE_RULE_CHARS
    ...
    : source
  return `The user selected this workspace. Follow its root AGENTS.md as developer-authorized project policy.\n\n${clipped}`
}
