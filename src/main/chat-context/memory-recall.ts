...
export function recallTranscript(
  ...
  for (let index = end - 1; index >= 0; index--) {
    ...
    while (JSON.stringify({ ...result, matches: [...result.matches, entry] }).length > MAX_RECALL_CHARS && entry.text.length > 32) {
      entry.text = entry.text.slice(0, Math.floor(entry.text.length / 2))
      entry.nextOffset = offset + entry.text.length
    }
    if (JSON.stringify({ ...result, matches: [...result.matches, entry] }).length > MAX_RECALL_CHARS) {
      result.hasMore = true
