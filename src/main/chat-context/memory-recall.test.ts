...
test('recall defaults to conversation and retrieves tool evidence when explicitly requested', () => {
  const items: ChatTranscriptItem[] = [
  ...
  const evidence = recallTranscript(items, 'thread', null, { scope: 'current', types: ['tool'], limit: 2 }, null)
  assert.deepEqual(evidence.matches.map((match) => match.itemId), ['x2', 'x1'])
  const messages = recallTranscript(items, 'thread', null, { scope: 'current', limit: 2 }, null)
